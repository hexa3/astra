...
<div class="shell" class:ai-open={state?.ai?.open} style={`--sidebar-width:${sidebarWidth(state?.sidebarCollapsed)}px;--ai-width:${state?.ai?.open ? AI_WIDTH : 0}px`}>
  ...
  <aside class="sidebar" class:collapsed={state?.sidebarCollapsed} id="browser-sidebar" aria-label="Tabs and library">
    ...
    <div class="sidebar-bottom">
      ...
      <button class="library-button" aria-label="Customize this site" title="Customize this site" class:selected={state?.panel === 'boosts'} onclick={() => run({ type: 'panel', value: state?.panel === 'boosts' ? 'none' : 'boosts' })}><Icon name="globe" /><span>Customize site</span></button>
  ...
  <main class="canvas" id="main-content">
    ...
    {:else if state?.panel === 'boosts'}
      <BoostPanel {state} {tab} {run} />
    {:else if tab?.error}
